...
export default function ExperimentDetail() {
  ...
  const calculateStats = () => {
    ...
    return {
      ...
      finalAccuracy: experimentResult?.accuracy
        ? experimentResult.accuracy
        : null,
      roundsCompleted: experimentResult?.rounds_completed || 0,
      totalRoundsConfig:
        experimentResult?.total_rounds || experiment?.parameters?.epochs || 0,
    };
  };
  ...
  const ClientResultRow = ({ clientId, clientData, allResults }) => {
    ...
    const sortedRounds = [...clientData.rounds].sort(
      (a, b) => a.round - b.round,
    );
    ...
    return (
      <>
        ...
        >
          ...
          <td className="px-6 py-4 whitespace-nowrap text-right">
            <span className="text-sm text-gray-900">{avgLoss.toFixed(4)}</span>
          </td>
          <td className="px-6 py-4 whitespace-nowrap text-right">
            <span className="text-sm text-gray-500">
              {clientData.lastUpdate
                ? new Date(clientData.lastUpdate).toLocaleDateString()
                : "-"}
            </span>
          </td>
        </tr>
                    ...
                    {sortedRounds.map((roundData, idx) => (
                      <tr key={idx} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-2 text-right text-sm text-gray-500">
                          {roundData.timestamp
                            ? new Date(roundData.timestamp).toLocaleString()
                            : "-"}
                        </td>
                      </tr>
                    ))}
                    ...
                    {experimentResult && (
                      <div className="p-3 bg-blue-50 rounded-xl">
                        ...
                        <p className="text-lg font-semibold text-blue-700">
                          {experimentResult.accuracy
                            ? experimentResult.accuracy.toFixed(2)
                            : "N/A"}
                          %
                        </p>
                      </div>
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full">
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {Object.entries(groupClientResults(clientResults)).map(
                          ([clientId, clientData]) => (
                            <ClientResultRow
                              key={clientId}
                              clientId={clientId}
