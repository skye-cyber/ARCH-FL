...
class ArchitectureGenerator:
    ...
    def __init__(
        self,
        config_dir: str = (
            Path(__file__).resolve().parent.parent.parent / "config/architectures/"
        ),
    ):
        """
    ...
    def _load_architecture_rules(self) -> None:
        ...
        if os.path.exists(rules_file):
            ...
            warnings.warn(
                f"Architecture rules file not found, using defaults: {rules_file}"
            )
    ...
    def _define_search_space(self) -> Dict[str, Any]:
        ...
        return {
            "conv_layers": {"min": 2, "max": 6, "step": 1},
            "conv_channels": {"min": 16, "max": 512, "step": 16, "growth_factor": 2.0},
            "fc_layers": {"min": 1, "max": 3, "step": 1},
            "fc_units": {"min": 64, "max": 1024, "step": 64},
            "stride": [1, 2],
            "kernel_size": [3, 5],
            "padding": [0, 1],
    ...
    def generate_architecture(
        ...
        try:
            ...
            if DATASET_ANALYZER_AVAILABLE and dataset_name:
                ...
                task_type = task_type or properties.get(
            ...
            if input_shape:
                ...
                config = self._generate_from_template(
                    arch_type, input_shape, task_type, num_classes
                )
            else:
                # Fallback to medium_cnn if no input shape
                config = self._generate_from_template(
                    "medium_cnn", (1, 224, 224), task_type, num_classes
                )
    ...
    def _generate_from_template(
        self, arch_type: str, input_shape: tuple, task_type: str, num_classes: int
    ) -> Dict[str, Any]:
        ...
        for i in range(num_conv_layers):
            if i < len(adjusted_depths):
                ...
                out_channels = adjusted_depths[-1] * (
                    2 ** (i - len(adjusted_depths) + 1)
                )
            ...
            conv_layers_config.append(
                {
                    ...
                    else 1,  # Reduce stride for last layer
                    "padding": 1,
                }
            )
        ...
        for i in range(num_fc_layers):
            ...
            fc_layers_config.append({"out_features": out_features})
    ...
    def _apply_constraints(
        ...
        if "max_memory_mb" in constraints:
            constrained_config = self._apply_memory_constraint(
                constrained_config, constraints["max_memory_mb"]
            )

        # Apply training time constraints
        if "max_training_time" in constraints:
            constrained_config = self._apply_training_time_constraint(
                constrained_config, constraints["max_training_time"]
            )

        # Apply parameter count constraints
        if "max_parameters" in constraints:
            constrained_config = self._apply_parameter_constraint(
                constrained_config, constraints["max_parameters"]
            )

        constrained_config["constraints_applied"] = constraints
        return constrained_config

    def _apply_memory_constraint(
        self, config: Dict[str, Any], max_memory_mb: float
    ) -> Dict[str, Any]:
        ...
        if current_memory > max_memory_mb:
            ...
            if len(conv_layers) > 2:
                # Remove some layers
                new_conv_layers = conv_layers[
                    : max(2, int(len(conv_layers) * reduction_factor))
                ]
                config["architecture"]["conv_layers"] = new_conv_layers

            # Reduce channel counts
            for layer in config["architecture"]["conv_layers"]:
                layer["out_channels"] = max(
                    16, int(layer["out_channels"] * reduction_factor)
                )
            ...
            for i, layer in enumerate(fc_layers[:-1]):  # Don't reduce final layer
                layer["out_features"] = max(
                    64, int(layer["out_features"] * reduction_factor)
                )

        return config

    def _apply_training_time_constraint(
        self, config: Dict[str, Any], max_training_time: float
    ) -> Dict[str, Any]:
    ...
    def _apply_parameter_constraint(
        self, config: Dict[str, Any], max_parameters: int
    ) -> Dict[str, Any]:
    ...
    def _estimate_parameter_count(self, config: Dict[str, Any]) -> int:
        ...
        for layer in conv_layers:
            ...
            conv_params += out_channels * (
                current_channels * kernel_size * kernel_size + 1
            )
    ...
    def _validate_architecture(
        self, config: Dict[str, Any], input_shape: tuple
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if not conv_layers or len(conv_layers) < 1:
                ...
                for i, layer in enumerate(conv_layers):
                    if "out_channels" not in layer:
                        validation_result["errors"].append(
                            f"Conv layer {i} missing out_channels"
                        )
                        validation_result["valid"] = False
                    if layer["out_channels"] < 8:
                        validation_result["warnings"].append(
                            f"Conv layer {i} has very few channels: {layer['out_channels']}"
                        )
            ...
            validation_result["estimated_parameters"] = self._estimate_parameter_count(
                config
            )
            validation_result["estimated_memory_mb"] = self._estimate_memory_usage(
                config
            )
            validation_result["estimated_training_time"] = self._estimate_training_time(
                config
            )
    ...
    def _generate_fallback_config(
        self, input_shape: Optional[tuple] = None, task_type: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "architecture": {
                ...
                "fc_layers": [{"out_features": 256}, {"out_features": num_classes}],
                "activation": "ReLU",
                "pooling": "MaxPool2d",
                "pool_kernel": 2,
                "dropout": 0.5,
    ...
    def generate_and_create_model(
        ...
        config = self.generate_architecture(
            dataset_name, input_shape, task_type, constraints
        )
    ...
    def neural_architecture_search(
        ...
        for trial_num in range(num_trials):
            ...
            config = self._generate_random_architecture_variation(
                dataset_name, input_shape, task_type, constraints
            )
    ...
    def _generate_random_architecture_variation(
        self,
        dataset_name: str,
        input_shape: tuple,
        task_type: str,
        ...
        base_config = self.generate_architecture(
            dataset_name, input_shape, task_type, constraints
        )
        ...
        for layer in arch["conv_layers"]:
            layer["out_channels"] = max(
                16, int(layer["out_channels"] * random.uniform(0.8, 1.2))
            )

        # Vary FC layer sizes (±20%)
        for i, layer in enumerate(arch["fc_layers"][:-1]):  # Don't vary final layer
            layer["out_features"] = max(
                64, int(layer["out_features"] * random.uniform(0.8, 1.2))
            )
    ...
    def _score_architecture(
        self, config: Dict[str, Any], validation: Dict[str, Any]
    ) -> float:
    ...
    def save_search_results(
        self, search_results: Dict[str, Any], output_file: str
    ) -> None:
    ...
    for dataset_name in datasets_to_test:
        ...
        try:
            ...
            test_input = torch.randn(
                1, *config["architecture"]["input_channels"], *config["image_size"]
            )
