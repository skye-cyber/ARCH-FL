"""
Implements update averaging logic
Implements:
    fed_avg - Federated Averaging
    weighted_aggregation - Weighted Averaging
    secure_aggregation - Note implemented
"""
...
def fed_avg(
    client_updates: List[Dict[str, torch.Tensor]], client_sizes: List[int]
...
def weighted_aggregation(
    client_updates: List[Dict[str, torch.Tensor]], weights: List[float]
) -> Dict[str, torch.Tensor]:
    ...
    logger.debug(
        f"Weighted aggregation with custom weights for {len(client_updates)} clients"
    )
    return averaged_params
